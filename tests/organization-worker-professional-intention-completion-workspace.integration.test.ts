...
describe(
  "Organization Worker detail + Professional Intention Completion Workspace",
  () => {
    ...
    it("loads the workspace through the bounded shared Admin endpoint", () => {
      ...
      // The Organization view is coaching support. A supplemental read
      // failure must not make the entire Worker detail unusable.
      expect(body).not.toContain("throw ");
    });

    it("loads completion support for the current worker", () => {
      expect(source).toContain(
        "loadProfessionalIntentionCompletionWorkspace",
      );

      expect(source).toMatch(
        /loadProfessionalIntentionCompletionWorkspace\s*\(\s*workerId\s*\)/,
      );
    });

    it("renders the clarification workspace for the Organization coach", () => {
      expect(source).toContain(
    ...
    it("does not turn Organization into employer truth or expose premature plan content", () => {
